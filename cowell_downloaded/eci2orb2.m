...
sma = 1 / (2 / rmag - vmag^2 / mu);

% keplerian period

period = pi2 / sqrt(mu / sma^3);
...
vperigee = sqrt(2 * mu * rapogee / (rperigee * (rapogee + rperigee)));

vapogee = sqrt(2 * mu * rperigee / (rapogee * (rapogee + rperigee)));
...
if (sma > 0)
   ...
   [ttpp, ttanc] = tevent (mu, sma, eccm, argper, tanom);
